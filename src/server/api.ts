...
export type Repo = { name: string; repo: string };

export type ResolvedRepo =
  | { error: { status: number; body: { error: string } } }
  | { project: Repo; token: string; envVar: string };

export function resolveRepo(projectId: string | null | undefined, env: Record<string, string>): ResolvedRepo {
...
export function composeBugTitle(rawTitle: string, tester?: string): string {
...
export function readJsonBody(req: any): Promise<BugPayload> {
...
export async function createBugIssue(payload: BugPayload, env: Record<string, string>) {
...
export async function listViaGraphql(project: Repo, token: string) {
...
export async function listViaRest(project: Repo, token: string) {
...
export async function fetchOpenPrMap(project: Repo, token: string) {
...
export async function fetchIssueRow(
  project: Repo,
  token: string,
  number: number,
...
export async function applyOpenPrs(project: Repo, token: string, issues: any[]) {
...
export async function listBugIssues(projectId: string | null, env: Record<string, string>) {
...
export async function updateBugIssue(payload: BugPayload, env: Record<string, string>) {
  const resolved = resolveRepo(payload.project, env);
  if ('error' in resolved) return resolved.error;
  const { project, token } = resolved;
